...
export function resolveRepoPath() {
    ...
    if (!raw) {
        console.error("Missing repo path. Usage: git-insights-mcp <path-to-git-repo>\n" +
...
export function getGit(repoPath) {
    if (!gitInstance) {
        ...
        throw new Error(`getGit() called with "${repoPath}" but the server is scoped to "${gitBaseDir}".`);
    }
    return gitInstance;
}
/**
 ...
 */
export function assertPathInsideRepo(repoPath, target) {
