...
type GetCommitDiffParams = z.infer<z.ZodObject<typeof getCommitDiffSchema>>;

export async function getCommitDiff(repoPath: string, params: GetCommitDiffParams) {
...
type GetDiffBetweenBranchesParams = z.infer<z.ZodObject<typeof getDiffBetweenBranchesSchema>>;

export async function getDiffBetweenBranches(
  repoPath: string,
  params: GetDiffBetweenBranchesParams,
) {
  const git = getGit(repoPath);
...
type GetWorkingTreeDiffParams = z.infer<z.ZodObject<typeof getWorkingTreeDiffSchema>>;

export async function getWorkingTreeDiff(repoPath: string, params: GetWorkingTreeDiffParams) {
  const git = getGit(repoPath);
